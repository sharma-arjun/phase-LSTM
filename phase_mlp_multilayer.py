...
class PMLP(nn.Module):
	def __init__(self, input_size, output_size, hidden_size, n_layers=1, batch_size=1):
		...
		if n_layers == 2:
			...
			self.l_01 = nn.Linear(self.hidden_size, self.hidden_size)
			self.l_11 = nn.Linear(self.hidden_size, self.hidden_size)
			self.l_21 = nn.Linear(self.hidden_size, self.hidden_size)
			self.l_31 = nn.Linear(self.hidden_size, self.hidden_size)
	...
	def forward(self,x,phase):
		...
		hiddens = []
		l0 = nn.Linear(self.input_size, self.hidden_size)
		hiddens.append(l0)
		h2o = nn.Linear(self.hidden_size, self.output_size)
		if self.n_layers == 2:
			l1 = nn.Linear(self.hidden_size, self.hidden_size)
			hiddens.append(l1)
	...
	def init_controls(self, list_of_hidden, list_of_h2o, alpha):
		for i in range(len(alpha)):
			for j in range(len(list_of_hidden)):
				...
				alpha[i]._parameters['weight_' + str(j)] = torch.from_numpy(np.copy(l._parameters['weight'].data.numpy()))
				alpha[i]._parameters['bias_' + str(j)] = torch.from_numpy(np.copy(l._parameters['bias'].data.numpy()))

				#initialize alpha grads as zero here using shape ...
				alpha[i]._grad['weight_' + str(j)] = torch.zeros(l._parameters['weight'].data.numpy().shape)
				alpha[i]._grad['bias_' + str(j)] = torch.zeros(l._parameters['bias'].data.numpy().shape)
